...
namespace webapicsharp.Repositorios
{
            ...
            private object ConvertirValor(string valor, NpgsqlDbType? tipoDestino)
            {
                ...
                try
                {
                    ...
                    return tipoDestino switch
                    {
                        ...
                        NpgsqlDbType.Time => ConvertirHora(valor),          // "12:30:45" → TimeOnly (C# 6+)
            ...
            private DateTime ConvertirTimestamp(string valor)
    {
        ...
        /// </summary>
            ...
            )
            {
                ...
                try
                {
                    ...
                    // === FILTRAR COLUMNAS AUTO-INCREMENTALES ===
                    ...
                    // Validar que queden datos después de filtrar
                    if (!datosFinales.Any())
                    {
                        throw new InvalidOperationException(
                    ...
                    // === EJECUCIÓN CON PARÁMETROS TIPADOS ===
                    await using var comando = new NpgsqlCommand(sql, conexion);
                    ...
                    foreach (var kvp in datosFinales)
                    {
                        ...
                        {
                            // Convertir valor al tipo apropiado si es string
                        ...
                        {
                            // Fallback: usar AddWithValue si no se pudo detectar el tipo
            ...
            // === MÉTODO AUXILIAR PARA DETECTAR COLUMNAS AUTO-INCREMENTALES ===
            ...
            )
            {
                ...
                // Query para detectar columnas SERIAL, BIGSERIAL, SMALLSERIAL e IDENTITY
                string sqlDeteccion = @"
        ...
        /// </summary>
 /// <summary>
/// Implementa UPDATE con detección de tipos para TODOS los parámetros (SET y WHERE).
/// Ahora aplica la misma lógica de detección de tipos que el método CrearAsync.
/// </summary>
        ...
        )
        {
            ...
            try
            {
                ...
                // === DETECTAR TIPOS DE COLUMNAS PARA SET ===
                var tiposColumnas = new Dictionary<string, NpgsqlDbType?>();
                foreach (var columna in datosFinales.Keys)
                {
                    var tipo = await DetectarTipoColumnaAsync(nombreTabla, esquemaFinal, columna);
                ...
                // === PARÁMETROS PARA SET CON TIPOS ESPECÍFICOS ===
                foreach (var kvp in datosFinales)
                {
                    string nombreColumna = kvp.Key;
                    object? valor = kvp.Value;
